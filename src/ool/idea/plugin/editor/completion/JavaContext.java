...
public class JavaContext extends CompletionContributor
{
    ...
    private final List<String> shippedBaseExtenderMethods = Arrays.asList(new String[]{
            "getObject", "getValue", "isVisibleProperty", "containsProperty", "getPropertyKeys", "toString"
    });
    ...
    public void fillCompletionVariants(@NotNull CompletionParameters parameters, @NotNull CompletionResultSet result)
    {
        ...
        if (type != null)
        {
            if (type instanceof JSCompositeTypeImpl)
            {
                for (JSType jsType : ((JSCompositeTypeImpl) type).getTypes())
                {
                    ...
                    {
                        possibleJavaTypes.add(jsType.getTypeText());
                    }
                }
            }
        ...
        for (String possibleType : possibleJavaTypes)
        {
            if ((aClass = facade.findClass(possibleType, scope)) != null)
            {
                ...
                {
                    possibleInstancesOf.add(extender);
                }
            }
        }
