...
public class ExtenderProvider
{
    ...
    private static Set<PsiClass> getExtenderProvidersFor(@NotNull final PsiClass original)
    {
        ...
        if (cached == null)
        {
            cached = CachedValuesManager.getManager(project).createCachedValue(new CachedValueProvider<HashMultimap<String, PsiClass>>()
            {
                @Nullable
                @Override
                public Result<HashMultimap<String, PsiClass>> compute()
                {
